...
from src.generation import generate_sequentially
...
def test_steam(
    ...
    save: bool = False,
    fp: str = "",
    gen_classifier: str = "logistic",
):
    """
    Assesses the performance of STEAM models with different classifiers.
    ...
    gen_classifier : str, optional
        The classifier used for the Q_W|X distribution in STEAM. Options are `'logistic'` (logistic regression) or `'rf'` (Random Forest). Default is `'logistic'`.

    Returns
    -------
    pandas.DataFrame
        A DataFrame containing the evaluation results for the STEAM method.
        The DataFrame has the following columns:
        - `'classifier'`: The classifier used in generation.
    ...
    results = pd.DataFrame(columns=["classifier", "jsd_pi", "u_pehe"])
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=["logistic", "rf"],
        default="rf",
        help="Classifier to use for Q_W|X in STEAM. Defaults to logistic regression.",
    )
...
def run_experiments(
    ...
    for dataset_name in datasets:
        ...
        print(f"Running {dataset_name.upper()} assessment")

        for model in models:
            print(f"  - Model: {model}")
            print(f"    - Classifier: {gen_classifier}")
            output_path = os.path.join(output_dir, f"classifier_ablation_{dataset_name}_{model}.csv")
            test_steam(
                real,
                model,
                treatment_col,
...
def main() -> None:
    args = parse_args()
    run_experiments(args.datasets, args.models, args.iterations, args.output_dir, args.classifier, args.gen_classifier)
